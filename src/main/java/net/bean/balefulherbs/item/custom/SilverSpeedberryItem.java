...
public class SilverSpeedberryItem extends Item {

    public SilverSpeedberryItem(Properties pProperties) {
    ...
    public ItemStack finishUsingItem(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity) {
        ...
        if (pLivingEntity instanceof Player player && !pLevel.isClientSide()) {
            player.getCapability(ModCapabilities.PLAYER_CRAVE_CAPABILITY).ifPresent(data -> {
                data.setConsumedTime("silver_speedberry", pLevel.getGameTime());
            });
        }
